...
import { createGitHubIssue, getConfiguredGitHubRepo } from '@/server/github/issues'
...
/** Browser sanity check; Discord only uses POST with signature headers. */
export async function GET() {
  const hasPublicKey = Boolean(process.env.DISCORD_PUBLIC_KEY)
  return NextResponse.json({
    ok: true,
    discordPublicKeyConfigured: hasPublicKey,
    message:
      'Discord Interactions endpoint. Use POST (Discord or PowerShell); opening this URL in a browser is GET only.',
  })
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const issue = await createGitHubIssue(issueInput, getConfiguredGitHubRepo())
