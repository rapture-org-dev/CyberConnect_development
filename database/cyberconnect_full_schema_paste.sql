...
-- ================================================================================
-- HOW TO USE:
--   1. Create a new Supabase project (or empty Postgres with auth.users).
--   2. Open SQL Editor and paste this entire file, OR run section-by-section.
--   3. If a statement fails (duplicate object), skip or comment it.
--   4. Enum labels are inferred from the repo; compare with production if migrating data.
--   5. This bundle does NOT include all RLS policies from Supabase dashboard - add as needed.
--   6. Run the ENTIRE script in order. Sheet RBAC triggers are only at rbac_sheet_triggers.sql
--      (do not stop before that section on a new database).
-- ================================================================================
